<?php
class Tobeit_Gdrt_Block_Script extends Mage_Core_Block_Abstract {
    ...
    protected function _toHtml()
    {
        ...
        $version = (string)Mage::getConfig()->getNode()->modules->Tobeit_Gdrt->version;
        
        $s = PHP_EOL .
            '<!-- Tobeit GDRT v.' . $version . ' script begin -->' . PHP_EOL .
            ...
            '</div>' . PHP_EOL .
            '</noscript>' . PHP_EOL .
            '<!-- Tobeit GDRT script end -->' . PHP_EOL;
        ...
        if ((int)Mage::getStoreConfig('gdrt/debug/show_info', $this->_storeId) === 1)
        {
            ...
            if (empty($ips[0]) || in_array(Mage::helper('core/http')->getRemoteAddr(), $ips))
            {
                $s .= PHP_EOL .
                    ...
                    '        <tr>' .
                    '           <td rowspan="' . (count($gcParams) + 1) . '" style="vertical-align:middle;padding-right:40px;"><h3 style="margin:0;">Tobeit GDRT debug v.' . $version . '</h3></td>' .
